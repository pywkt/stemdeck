...
export function createChunkedAudioEngine(stems, { onTime, onEnded, context } = {}) {
  ...
  //
  // The processor decides whether a semitone is in use by whether its input
  // has any channels, and it takes its bypass path (a straight copy) only when
  // none do. Wiring every bus up front defeated that: a bus with nothing
  // playing into it still arrives as one channel of silence in Chrome, so the
  // processor built a pitch chain for all twelve semitones and ran WSOLA on
  // silence for the whole track, at zero transpose. Measured at 13% of real
  // time for seven stems on a desktop, against 0.6% with only the unpitched
  // input wired -- enough to starve the audio thread on a laptop and be heard
  // as pops. Connecting a bus only while a lane is on it hands the processor
  // the empty input its contract describes.
  ...
  // out again once empty, so the processor sees exactly the inputs that carry
  // a lane. The unpitched bus stays wired whatever its count, because the
  // click is scheduled onto it directly. In the tape-effect fallback there is
  // no worklet and every bus already feeds master, so there is nothing to do.
  //
  // Arrival is announced before the lane connects, so a bus is live in the
  // worklet by the time anything reaches it, and nothing watching the graph
  // sees a lane land on a bus that leads nowhere.
